...
const PostVideoUI = ({ setPopUp }) => {
  ...
  const { mutate: postMutation } = useMutation({
    mutationFn: async () => {
      if (mode === 'url')
        return await postVideoFromUrl(url, title, subtitle, aspectRatio);
      if (mode === 'file')
        return await postVideoFromFile(file, title, subtitle);
    },
    ...
    <form className={style.popup} onSubmit={submitPostHandler}>
      ...
      <div className={style.main}>
        ...
        <SubInput
          ...
          placeholder={'Optional: Aspect ratio (Ej.: 16:9)'}
        />

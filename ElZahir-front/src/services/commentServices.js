...
export async function fetchComments (post) {
    ...
    try {
        return await axios.get(vercelAPI.concat('/api/comment'), { params: { postId: post.id } })
    } catch (error) {
        console.error(error)
    }
...
export async function fetchSubComments (comment) {
    ...
    try {
        return await axios.get(vercelAPI.concat('/api/comment'), { params: { commentId: comment.id } })
    } catch(error) {
        console.log(error)
    }
}
...
export async function deleteComment (variables) {
    ...
    try {
        return await axios.delete(vercelAPI.concat(`/api/comment/${variables.comment.id}`), config);
    } catch (error) {
        console.error(error);
    }
};
...
export async function getUser(userID) {
  try {
    return await axios.get(
      vercelAPI.concat(`/api/users/${userID}`),
      getConfig()
    );
  } catch (error) {
    console.error(error);
...
export async function followUser(user, otherUser) {
  try {
    return await axios.put(
      `${vercelAPI}/api/users/follow/${otherUser.id}`,
      { id: user.id },
      getConfig()
    );
  } catch (error) {
...
export async function unfollowUser(user, otherUser) {
  try {
    return await axios.put(
      `${vercelAPI}/api/users/unfollow/${otherUser.id}`,
      { id: user.id },
      getConfig()
    );
  } catch (error) {
...
export async function changeProfileImageFromUrl(user, url) {
  try {
    ...
    return await axios.put(
      vercelAPI.concat(`/api/users/profile-image/${user.id}`),
      { profileImg: url, mode: 'profileImgURL' },
      config
    );
  } catch (error) {
    console.error(error);
...
export async function changeProfileImageFromFile(user, file) {
  try {
    ...
    return await axios.put(
      renderAPI.concat(`/api/users/profile-image/${user.id}`),
      formData,
      config
    );
  } catch (error) {
...
export async function changeBackgroundImgFromUrl(user, url) {
  try {
    ...
    return await axios.put(
      vercelAPI.concat(`/api/users/profile-panel-image/${user.id}`),
      { mainPanelImg: url, mode: 'panelImgUrl' },
      config
    );
  } catch (error) {
    console.error(error);
...
export async function changeBackgroundImgFromFile(user, file) {
  try {
    ...
    return await axios.put(
      renderAPI.concat(`/api/users/profile-panel-image/${user.id}`),
      formData,
      config
    );
  } catch (error) {

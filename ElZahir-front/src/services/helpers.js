...
export function isImageInputValid (url, file, mode) {
    ...
    const urlIsValid = (url.endsWith('.jpg') || url.endsWith('.png') || url.endsWith('.bmp') || url.endsWith('.gif') || url.endsWith('.webp') || url.endsWith('.tiff')) && url.length > 0?
    true : false

    const fileIsValid = file && (file.type?.startsWith('image/'))?
    true: false
...
export function isVideoInputValid (url, file, mode) {
    ...
    const urlIsValid = (url.endsWith('.webm') || url.endsWith('.mp4') || url.endsWith('.mov') || isYoutubeUrl(url) ) && url.length > 0 ? 
    true : false

    const fileIsValid = file && (file.type.startsWith('video/'))?
    true: false

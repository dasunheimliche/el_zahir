  ...
  return (
    <FullscreenPostBackground isFullscreen={isFullscreen}>
      <div
        className={
          isFullscreen
            ? `${style['mobile-container']} ${style.resizeHandler}`
            : undefined
        }>
        {isFullscreen && <div className="logo post-logo">Zahir.</div>}
        <PostWrapper isFullscreen={isFullscreen}>{children}</PostWrapper>
      </div>
...
export function ImagePostWrapper({ children, post, isFullscreen }) {
  ...
  const [ancho, setAncho] = useState(
    (size.width / size.height) * window.innerHeight
  );
  ...
  return (
    <FullscreenPostBackground isFullscreen={isFullscreen}>
      <div
        className={isFullscreen ? style['mobile-image-container'] : undefined}
        style={
          size.height > window.innerHeight - 150 && isFullscreen
            ? { width: `${ancho}px`, maxWidth: '90vw' }
            : undefined
        }>
        {isFullscreen && <div className="logo post-logo">Zahir.</div>}
        <PostWrapper isFullscreen={isFullscreen}>{children}</PostWrapper>
...
export function VideoPostWrapper({ children, post, isFullscreen }) {
  ...
  const aspectRatioNumber =
    post.type === 'video' &&
    (Number(aspectRatioString[1]) / Number(aspectRatioString[0])) * 100;

  return (
    <FullscreenPostBackground isFullscreen={isFullscreen}>
      <div
        className={isFullscreen ? style['mobile-container'] : undefined}
        style={
          isFullscreen
            ? aspectRatioNumber >= 100
              ? aspectRatioNumber >= 170
                ? { width: '23%' }
                : { width: '32%' }
              : { width: '50%' }
            : {}
  ...
  return (
    <FullscreenPostBackground isFullscreen={isFullscreen}>
      <div
        className={
          isFullscreen
            ? `${style['mobile-container']} ${style.videoFileResizeHandler}`
            : undefined
        }>
        {isFullscreen && <div className="logo post-logo">Zahir.</div>}
        <PostWrapper isFullscreen={isFullscreen}>{children}</PostWrapper>

...
function Comment ({comment, user, post, isMutating, onAnswerComment, onDeleteComment}) {
    return(
        <>
            ...
            <div className={style['comment-socials']}>
                <div className="p" onClick={onAnswerComment}>Answer</div>
                {comment.username === user.username || post.username === user.username? <div className="p" onClick={isMutating? undefined : onDeleteComment}>Delete</div> : console.log}
            </div>
        </>
    )
}
        ...
        return subComments.map((comment0, i) => {
            return (
                <Comment 
                    ...
                    onAnswerComment={()=>onTargetUserComment(comment.id, comment0.username)}
                    onDeleteComment={()=>handleDeleteComment({post, comment: comment0})}
                />
            )
        })
    }
    ...
    return (
        <div className={style['comment-container']}>
            <Comment 
                ...
                post={post}
                isMutating={isMutating}
                onAnswerComment={()=>onTargetUserComment(comment.id, comment.username)}

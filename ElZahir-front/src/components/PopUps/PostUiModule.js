...
export function SubInput({ value, onChange, placeholder }) {
  return (
    <>
      <input
        className={`${style.input} ${style.subtitle}`}
        placeholder={placeholder}
        onChange={onChange}
        value={value}
        autoComplete="off"
...
export function TextArea({ value, onChange }) {
  return (
    <>
      <textarea
        className={`${style.input} ${style['url-textarea']}`}
        placeholder="Text"
        onChange={onChange}
        value={value}
        autoComplete="off"
  ...
  return (
    <div className={style.uploader}>
      <textarea
        className={
          isInputValid
            ? `${style.input} ${style['file-textarea']} ${style.invalidTextAreaInput}`
            : `${style.input} ${style['file-textarea']} ${style.validTextAreaInput}`
        }
        placeholder={'File'}
        value={url}
        autoComplete="off"
      ...
      <div className={style.options}>
        <button
          className={`${style.uploadButton} ${style.clipboard} `}
          onClick={onPasteUrl}
        />
...
export function Error({ error }) {
  return <div>{error && <div className={style.error}>{error}</div>}</div>;
}
...
export function PostUiFooter({ onCancel, isMutating, isPostButtonDisabled }) {
  ...
  return (
    <div className={style.footer}>
      ...
      <button className={`${style.button} p`} type="button" onClick={onCancel}>
        CANCEL
      </button>
      <button className={`${style.button} p`} disabled={isButtonDisabled}>
        POST
      </button>
    </div>
  );

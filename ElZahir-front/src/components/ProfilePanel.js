...
import { FollowStats, FollowStatus, PostingOptions, ProfileImg, TopCardOptions } from "./ProfileCardModule"
    ...
    return (
        <div className={sticky === false? style.userCard : `${style.userCard} ${style['sticky-userCard']}`}>
            ...
            <div className={style.top} style={{"backgroundImage":`url(${mode === 'user'? otherUser.mainPanelImg : user.mainPanelImg})`}} >
                ...
                <TopCardOptions mode={mode} onChangeCoverImg={handleShowChangeProfileCoverMenu} />
            </div>

            <div className={isMine? style.bottom : ` ${style.bottom} ${style.userBottom}`}>
                <ProfileImg mode={mode} user={user} otherUser={otherUser} onChangeProfileImg={handleShowChangeProfileImgMenu}/>
                <FollowStatus mode={mode} isUserFollowed={isUserFollowed} isMutating={isMutating} onToggleFollow={handleToggleFollow} />
                <FollowStats mode={mode} user={user} otherUser={otherUser} posts={posts} onShowFollowers={()=>setPopUp({type: 'seeFollowers' , post: null})} onShowFollowing={()=>setPopUp({type: 'seeFollowings' , post: null})} />
